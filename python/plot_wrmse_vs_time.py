...
def main():
    ...
    run(BMK_PATH / "bmk6.csv", prefix="rmc")
...
def plot_wrmse_vs_pitch_and_roll(df):
    ...
    plot_data = (
        df_best.groupby(["cam_pitch_deg", "cam_roll_deg"])["frame_norm_weighted_rmse"]
        .mean()
        .reset_index()
    )
...
def plot_yaw_error_vs_pitch_and_roll(df):
    ...
    # We group by orientation to get the average 'best' yaw for that specific coordinate.
    plot_data = (
        df_best.groupby(["cam_pitch_deg", "cam_roll_deg"])["abs_yaw_error"]
        .mean()
        .reset_index()
    )
...
def plot_wrmse_over_yaw(df, fi=0, oi=0):
    ...
    df_filtered["normalized_wrmse"] = (df_filtered["weighted_rmse"] - min_val) / (
        max_val - min_val
    )
...
def plot_wrmse_distribution(df, oi=0):
    ...
    df_filtered["normalized_wrmse"] = (df_filtered["weighted_rmse"] - rmin) / (
        rmax - rmin
    )

    # 3. Pivot the data
    pivot_df = df_filtered.pivot(
        index="yaw_offset_deg", columns="datetime_utc", values="normalized_wrmse"
    )
    ...
    if not time_deltas.empty:
        ...
        full_time_range = pd.date_range(
            start=pivot_df.columns.min(), end=pivot_df.columns.max(), freq=min_delta
        )
...
def read_results(path):
    ...
    min_rmse_per_group = df.groupby(["frame_index", "orientation_index"])[
        "weighted_rmse"
    ].transform("min")
    ...
    df["frame_norm_weighted_rmse"] = (df["weighted_rmse"] - min_rmse_per_frame) / (
        max_rmse_per_frame - min_rmse_per_frame
    )
